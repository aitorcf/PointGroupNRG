...
function Eg_states( shell::Integer )
    # input:
...
function T2g_states( shell::Integer )
    # input:
    # - shell: number of the shell for which to 
    #          compute the states (usually=0)
    # output:
...
# C4v
function A1B1_states( shell::Integer ) 
    # input:
    # - shell: number of the shell for which to 
    #          compute the states (usually=0)
    # output:
...
function A1_states( shell::Integer )
    # input:
    # - shell: number of the shell for which to 
    #          compute the states (usually=0)
    # output:
...
function B1_states( shell::Integer )
    # input:
    # - shell: number of the shell for which to 
    #          compute the states (usually=0)
    # output:
...
function shell_hilbertstates( orbital::String , r::Int64 , dimension::Int64 )
    root = ( r , "orbital" )
    statetuples = [] 
...
function read_asymfile_fockbasis( filename ; verbose=false )
...
function read_asymfile_fockbasis_sage( filename )
    # read N3.txt kind of file and return a dictionary
    #       qnums => vector
    d = Dict()
    for line in readlines(filename)
...
function get_multiplets( symstates )
    m::Set{Tuple{Int64, String, Float64, Int64}} = Set( (k[1],k[2],k[3],k[end]) for k in keys(symstates) )
    return m
end

function get_irreps( symstates::Dict ; multiplicity=false )
...
function get_irreps( multiplets::Set ; multiplicity=false )
    # gets irreps (with multiplicities, in tuple form) 
    # of "symstates" or other Dict(q=>any) object.
...
function get_irrepstates( symstates , irrep )
    # returns symstates belonging the specified irrep
    return Dict{ Tuple{Int64,String,Float64,Int64,Float64,Int64} , State }( q=>s for (q,s) in symstates if q[1:3]==irrep )
end

function get_irrepstates_onepartner( symstates , irrep ; partner="o1_smax" )
    # return symstates corresponding to a chosen partner 
    # of the specified irrep 
    if partner=="o1_smax"
        return Dict{ Tuple{Int64,String,Float64,Int64,Float64,Int64} , State }( q=>s for (q,s) in get_irrepstates(symstates,irrep) if (q[4],q[5])==(1,q[3]) )
    end
end

function get_partners( symstates ; irreps=false , multiplicity=false ) 
...
get_partners_of_irrep( symstates , irrep ) =
    Set( k[1:5] for k in keys(symstates) if k[1:3]==irrep )

function get_multiplicity( symstates::Dict , irrep ) 
    return length(collect(m for m in get_multiplets(symstates) if m[1:3]==irrep))
end
function get_multiplicity( multiplets::Set , irrep )
...
function get_cg_o_fullmat( cg_o_fulldict )
    ...
    cg_o_fullmat = Dict{Tuple{String,String,String},Array{ComplexF64,3}}() 
...
function get_cg_o_fullmatint( cg_o_fulldict , oirreps )
    # Given a CG coefficient dictionary in the form 
    #
    #           cg[I_1,i_1,I_2,i_2,I_3,i_3] = ( I_1 , i_1 ; I_2 , i_2 | I_3 , i_3 ),
    #
    ...
    combs = Set( (k[1],k[3],k[5]) for k in keys(cg_o_fulldict) )
    cg_o_fullmat = Dict{Tuple{Int64,Int64,Int64},Array{ComplexF64,3}}() 
...
function cg_spin( S_1::Float64 , S_2::Float64 )
    cg_s::Dict{ NTuple{6,Float16} , ComplexF64 } = Dict{ NTuple{6,Float16} , ComplexF64 }()
...
function cg_combination( reduced_combination , oh_path )
    ...
    (I_0,S_0) = reduced_combination[1][2:end]
    (I_1,S_1) = reduced_combination[2][2:end]
    cg_o = cg_orbital( I_0 , I_1 , oh_path )
    cg_s = cg_spin( S_0 , S_1 )
    return tuple(cg_o,cg_s)
end
...
function symstates_n_oneirrep( basis , n , hiztegia , symstates=Dict() ;
         filename="/home/aitor/Kalkuloak/AntiSymmetricPart/Eg/N$n.txt" ,
         identity="A1g" )
    # input:
    ...
    if n==0  
        get!( symstates , tuple([0,identity,0,1,0,1]...) ,
              GenericBasis(basis).states[1] ) 
        return symstates
    elseif n==1 
        for sfs in basis.states
            (g,I,mu,m) = sfs.hilbert.states[ findfirst(sfs.occ) ]
            get!( symstates , tuple(1,hiztegia[I],1/2,mu,hiztegia[m],g) , 
                  State(sfs,basis) )
        end
        return symstates
    end
    fileinfo = read_asymfile_fockbasis( filename )
    for (sym,v) in fileinfo
        s = State( v , basis )
        ssym = [ sym[1] , map( x->eval(Meta.parse(x)) , sym[2:end] )... ]
        ssym[end] -= 1  # r=1 --> r=0 for r=N=0 in the innermost shell
        #ssym[4] /= 2 #OLD, for sage-generated files
        get!( symstates , tuple([n,ssym...]...) , s )
    end
    return symstates
end
...
function oneirrep_symstates( hilbert , hiztegia , identityrep , irrepath )
    basis = CanonicalBasis( hilbert )
    basis_number = [CanonicalBasis(hilbert,i) for i=0:length(hilbert.states)]

    symstates = Dict()
    for (i,b) in enumerate(basis_number)
        n = i-1
        symstates = symstates_n_oneirrep( b , n , hiztegia , symstates ;
                        filename=irrepath*"N$n.txt" , identity=identityrep )
    end
...
function sfs2coperator( sfs , basis )
...
function u_sym( symstates , symparams ; verbose=true )
    if verbose 
    ...
    for (q1,s1) in symstates_n2, (q2,s2) in symstates_n2 
        !check_atomic( s1 ) && continue
        !check_atomic( s2 ) && continue
...
get_irrEU_clean( identityrrep ) = 
    Dict{ Tuple{Int64,String,Float64} , Tuple{Vector{Float64},Matrix{ComplexF64}} }( 
        (0,identityrrep,0)=>( Float64[0.0] , ComplexF64[0.0im][:,:] ),
    )
