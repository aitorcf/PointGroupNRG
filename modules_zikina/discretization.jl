...
function compute_xi_vector( N::Int64 , 
    ...
    LL = BigFloat(L)
    N7 = minimum((7,N))
    ϵ = compute_epsilon_vector(N7,zz,LL;verbose=verbose,discretization=discretization)
    if discretization=="standard"
        return vcat( [ϵ[n]/ebar(z,L,n-1) for n=1:N7] , [1 for _=(N7+1):N] )
    elseif discretization=="co2005" 
        eco0 = compute_epsilon_co2005(8,z,L)[9]*L^4
        ebars = [eco0*L^(-n/2) for n=0:N]
        return vcat( [ϵ[i]/ebars[i] for i=1:N7] , [1 for _=(N7+1):N] )
    end
end

function compute_epsilon_co2005( N::Int64 , 
                                 z::R ,
                                 L::R ; 
...
function compute_epsilon_vector( N::Int64 , 
                                 z::BigFloat , 
                                 L::BigFloat ; 
                                 discretization="standard" ,
                                 verbose=true ) 
